...
public class TopicCommentService extends CrudService<TopicCommentEntity> {
    ...
    public TopicCommentEntity create(TopicCommentEntity topicCommentEntity) throws TipsException {
        if (EmptyUtil.isEmpty(topicCommentEntity.getCommentHead())
                || EmptyUtil.isEmpty(topicCommentEntity.getCommentName())
            ...
            throw new TipsException("发表评论时头像、昵称、创建人id、话题id、评论内容、父评论id不可为空");
        }
